...
const RegisterDPL = () => {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('registerDPLForm');
    if (saved) {
      try {
        setForm(JSON.parse(saved));
      } catch (e) {}
    }
    const savedStep = sessionStorage.getItem('registerDPLStep');
    if (savedStep === 'true') {
  ...
  useEffect(() => {
    sessionStorage.setItem('registerDPLForm', JSON.stringify(form));
    sessionStorage.setItem('registerDPLStep', showPaymentSection.toString());
  ...
  const handleSubmit = async (e) => {
    ...
    const uniqueFileName = `DPL_${cleanPhone}_${timestamp}.${extension}`;
      ...
      try {
        ...
        if(result.status === 'success') {
           setModalConfig({
             ...
             isError: false,
             onClose: () => {
                sessionStorage.removeItem('registerDPLForm');
                sessionStorage.removeItem('registerDPLStep');
                navigate('/');
             }
           });
        } else {
    ...
    >
      ...
      <div className="relative z-10 grow flex flex-col items-center justify-center px-4 py-24 sm:py-32">
        ...
        >
          ...
          >
            "Draft. Play. Lead." — Secure your spot now.
          </motion.p>
