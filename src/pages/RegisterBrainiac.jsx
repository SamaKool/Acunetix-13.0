...
const RegisterBrainiac = () => {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('registerBrainiacForm');
    if (saved) {
      try {
        setForm(JSON.parse(saved));
      } catch (e) {}
    }
    const savedStep = sessionStorage.getItem('registerBrainiacStep');
    if (savedStep === 'true') {
  ...
  useEffect(() => {
    sessionStorage.setItem('registerBrainiacForm', JSON.stringify(form));
    sessionStorage.setItem('registerBrainiacStep', showPaymentSection.toString());
  ...
  const handleSubmit = async (e) => {
    ...
    const uniqueFileName = `BRAINIAC_${cleanPhone}_${timestamp}.${extension}`;
      ...
      try {
        ...
        if(result.status === 'success') {
           setModalConfig({
             ...
             isError: false,
             onClose: () => {
                sessionStorage.removeItem('registerBrainiacForm');
                sessionStorage.removeItem('registerBrainiacStep');
                navigate('/');
             }
           });
        } else {
    ...
    >
      ...
      <div className="relative z-10 grow flex flex-col items-center justify-center px-4 py-24 sm:py-32">
        ...
        >
          ...
          >
            "Think Fast, Think Smart" — Secure your spot now.
          </motion.p>
          ...
          <form onSubmit={handleSubmit} className="register-form" autoComplete="off">
            ...
            <motion.div className="register-field-group" custom={0} initial="hidden" animate="visible" variants={fieldVariants}>
              <label htmlFor="register-name" className="register-label text-[#00ffc8]">Team Leader Name *</label>
...
export default RegisterBrainiac;
