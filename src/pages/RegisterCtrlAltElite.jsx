...
const RegisterCtrlAltElite = () => {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('registerCAEForm');
    if (saved) {
      try {
        setForm(JSON.parse(saved));
      } catch (e) {}
    }
    const savedStep = sessionStorage.getItem('registerCAEStep');
    if (savedStep === 'true') {
  ...
  useEffect(() => {
    sessionStorage.setItem('registerCAEForm', JSON.stringify(form));
    sessionStorage.setItem('registerCAEStep', showPaymentSection.toString());
  ...
  const handleSubmit = async (e) => {
    ...
    const uniqueFileName = `CAE_${cleanPhone}_${timestamp}.${extension}`;
      ...
      try {
        ...
        if(result.status === 'success') {
           setModalConfig({
             ...
             isError: false,
             onClose: () => {
                sessionStorage.removeItem('registerCAEForm');
                sessionStorage.removeItem('registerCAEStep');
                navigate('/');
             }
           });
        } else {
    ...
    >
      ...
      <div className="relative z-10 grow flex flex-col items-center justify-center px-4 py-24 sm:py-32">
        ...
        >
          ...
          >
            "Reboot. Rewrite. Reign." — Secure your spot now.
          </motion.p>
...
export default RegisterCtrlAltElite;
