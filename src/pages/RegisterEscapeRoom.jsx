...
const RegisterEscapeRoom = () => {
  ...
  useEffect(() => {
    const saved = sessionStorage.getItem('registerEscapeForm');
    if (saved) {
      try {
        setForm(JSON.parse(saved));
      } catch (e) {}
    }
    const savedStep = sessionStorage.getItem('registerEscapeStep');
    if (savedStep === 'true') {
  ...
  useEffect(() => {
    sessionStorage.setItem('registerEscapeForm', JSON.stringify(form));
    sessionStorage.setItem('registerEscapeStep', showPaymentSection.toString());
  ...
  const handleSubmit = async (e) => {
    ...
    const uniqueFileName = `ESCAPE_${cleanPhone}_${timestamp}.${extension}`;
      ...
      try {
        ...
        if(result.status === 'success') {
           setModalConfig({
             ...
             isError: false,
             onClose: () => {
                sessionStorage.removeItem('registerEscapeForm');
                sessionStorage.removeItem('registerEscapeStep');
                navigate('/');
             }
           });
        } else {
    ...
    >
      ...
      <div className="relative z-10 grow flex flex-col items-center justify-center px-4 py-24 sm:py-32">
        ...
        >
          ...
          >
            "There Is No Way Out" — Secure your spot now.
          </motion.p>
...
export default RegisterEscapeRoom;
